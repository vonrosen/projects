...
public class ShortestPathColors2 {
    ...
    public static void main(String[] args) {
        ShortestPathColors2 s = new ShortestPathColors2();
//        int n = 9;
...
//        };
    ...
    public int[] shortestAlternatingPaths(int n, int[][] red_edges, int[][] blue_edges) {    		
    		Map<Integer, List<Integer>> redGraph = new HashMap<Integer, List<Integer>>();
    		Map<Integer, List<Integer>> blueGraph = new HashMap<Integer, List<Integer>>();
    		
    		for (int [] edge : red_edges) {
    			int from = edge[0];
    			int to = edge[1];
    			
    			redGraph.putIfAbsent(from, new ArrayList<Integer>());
    		...
    		for (int [] edge : blue_edges) {
    			...
    			blueGraph.putIfAbsent(from, new ArrayList<Integer>());
    			blueGraph.get(from).add(to);
    		}
    	
    		Queue<int []> graph = new LinkedList<int []>();
    		...
    		graph.offer(new int [] {0, 0});
    		graph.offer(new int [] {0, 1});

    		while (!graph.isEmpty()) {
    			int size = graph.size();
    			
    			while (size-- > 0) {
    				int [] entry = graph.poll();
    				int from = entry[0];
    				int color = entry[1];
    				
    				if (visited[color][from]) {
    					continue;
    				}
    				...
    				visited[color][from] = true;
    				
    				Map<Integer, List<Integer>> currentGraph = color == 0 ? redGraph : blueGraph;
    				
    				if (currentGraph.get(from) != null) {
        				for (int to : currentGraph.get(from)) {    					
        					graph.offer(new int [] { to, color == 0 ? 1 : 0 });
        				}    					
    				}
    			}
