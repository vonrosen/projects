...
//		};
	...
	public int minFallingPathSum(int[][] A) {
		Integer [][] mem = new Integer[A.length][A.length];
		int minSum = Integer.MAX_VALUE;

		for (int c = 0; c < A.length; ++c) {
			int sum = sum(0, c, A, mem);
	...
	public int sum(int r, int c, int [][] A, Integer [][] mem) {
		if (r + 1 >= A.length) {
			return A[r][c];
		}
		...
		for (int newC = c - 1; newC <= c + 1; ++newC) {
			if (newC < A.length && newC >= 0 && r + 1 < A.length) {
				int sum = 0;
				if (mem[r + 1][newC] != null) {
					sum = A[r][c] + mem[r + 1][newC];
				}
				else {
					sum = sum(r + 1, newC, A, mem);
					mem[r + 1][newC] = sum;
					sum += A[r][c];
				}
